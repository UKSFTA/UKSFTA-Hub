DATE: 16 February 2026
CLASSIFICATION: RESTRICTED / NATO CONFIDENTIAL
EVALUATOR: --------------------------
CANDIDATE NAME/ID: --------------------------

-

### SCORING RUBRIC
- [ ] FAIL: Incorrect procedure; high risk of mission compromise/casualties.
- [ ] PASS: Basic doctrinal knowledge; mission success likely.
- [ ] EXCEEDS: High technical proficiency; demonstrates advanced "Recce" mindset.

-

### SESSION 1: INFILTRATION & INTELLIGENCE
#### MODULE A: THE STALKING PHASE
| Criteria                         | Score (0-2) | Observations / Notes |
| :------------------------------- | :---------: | :------------------- |
| Stealth & Thermal Signature MGMT |             |                      |
| Micro-terrain Usage              |             |                      |
| Speed vs. Signature Balance      |             |                      |
**Critical Fail:** Visual compromise or "Noise Negligence" (disturbing local wildlife/foliage).

#### MODULE B: DATA ACQUISITION / ISR
| Criteria | Score (0-2) | Observations / Notes |
| :--- | :---: | :--- |
| PIR Identification Accuracy (5/5) | | |
| Intelligence Report Quality (B-D-R-T) | | |
| OP Concealment (Lens Glint/Movement) | | |
**Critical Fail:** Missing 2 or more primary PIRs or "Lens Glint" detection.

-

### SESSION 2: OVERWATCH & EXFILTRATION
#### MODULE C: PRECISION SPOTTER
| Criteria | Score (0-2) | Observations / Notes |
| :--- | :---: | :--- |
| Verbal Clarity & Comms Procedure | | |
| Ranging & Ballistic Accuracy | | |
| Correction Speed (within 3s) | | |
**Critical Fail:** Failure to provide a follow-up correction within 3 seconds of a missed "splash."

#### MODULE D: TACTICAL BREAK-CONTACT
| Criteria                             | Score (0-2) | Observations / Notes |
| :----------------------------------- | :---------: | :------------------- |
| Navigation & PACE Plan Adherence     |             |                      |
| Speed vs. Stealth Balance            |             |                      |
| ERV Link-up (3min Extraction Window) |             |                      |
**Critical Fail:** Capture by tracker team or failure to adhere to the PACE plan navigation route.

-

### EVALUATOR’S SUMMARY

**TOTAL SCORE:** ---- / 24  
**MINIMUM PASSING SCORE:** 17/24

**RECOMMENDATION:**
[ ] QUALIFIED (Ready for Operational Deployment)
[ ] RE-TRAINING REQUIRED (Specific Modules)
[ ] DISMISSED FROM SELECTION

**COMMENTS:**
--------------------------------------------------------------------------
--------------------------------------------------------------------------

-

Signed,
--------------------------
(Section Leader)
Special Reconnaissance Regiment
